...
export default function BuilderContent({ planId }) {
  ...
  const loadPlan = useCallback(async () => {
    try {
      const res = await fetch(`/api/builder/${planId}`);
      if (res.ok) {
  ...
  async function planAction(action) {
    ...
    try {
      await fetch(`/api/builder/${planId}`, {
        method: 'POST',
  ...
  async function handleRetry(stepId) {
    setActionLoading(true);
    try {
      // Reset this step and re-run from the API
      await fetch(`/api/builder/${planId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex gap-2 ml-4">
          {(plan.status === 'planned' || plan.status === 'planning') && (
            <button
              onClick={() => planAction('start')}
              disabled={actionLoading}
              className="px-4 py-2 bg-primary text-background rounded-lg font-medium hover:bg-primary/90 disabled:opacity-50 transition-colors cursor-pointer"
            >
              Start Build
            </button>
          )}
          {plan.status === 'running' && (
            <button
              onClick={() => planAction('pause')}
              disabled={actionLoading}
              className="px-4 py-2 bg-amber-500/20 text-amber-400 rounded-lg font-medium hover:bg-amber-500/30 disabled:opacity-50 transition-colors cursor-pointer"
            >
              Pause
            </button>
          )}
          ...
          <button
            ...
            className="px-4 py-2 bg-red-500/10 text-red-400 rounded-lg font-medium hover:bg-red-500/20 transition-colors cursor-pointer"
          >
            Delete
          </button>
        </div>
            ...
            >
              ...
              {step.status === 'failed' && (
                <button
                  ...
                  disabled={actionLoading}
                  className="px-3 py-1 text-xs bg-primary/20 text-primary rounded-lg hover:bg-primary/30 transition-colors cursor-pointer"
                >
                  Retry
                </button>
              )}
